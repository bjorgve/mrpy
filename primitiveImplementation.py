...
def waveletScaled(order, scale, translation, nr_poly, x):
        return 2**(scale/2) \
            * waveletScaleZero(order, nr_poly, (2**scale)*x-translation)
...
def approximatedScalingCoef(order, scale, translation, f):
    ...
    x = arange(0, 1, .01)
    return 2**(-.5*scale)*trapz(
            legendre_polynomial(order, x)*f(2**(-scale)*(x + translation)),
            dx=0.01)
...
def normWavletScaleZero(order_wavelet_one, order_wavelet_two):
    ...
    return trapz(
            waveletScaleZero(order_wavelet_one, 4, x)
            * waveletScaleZero(order_wavelet_two, 4, x), dx=dx)


def normWavletScale(order_wavelet_one, scale_wavelet_one, order_wavelet_two,
                    scale_wavelet_two, translation_one, translation_two):
    from numpy import arange, trapz
    dx = 0.001
    x = arange(0., 1., dx)
    ...
    return trapz(
        waveletScaled(
            order_wavelet_one, scale_wavelet_one, translation_one, 4, x)
        * waveletScaled(
            order_wavelet_two, scale_wavelet_two, translation_two, 4, x),
        dx=dx)
